...
import { n as isAcpSessionKey, o as parseAgentSessionKey } from "./session-key-utils-CyEfEYg-.js";
import { c as normalizeAgentId, l as normalizeMainKey } from "./session-key-C01Y8-9J.js";
import { r as logVerbose } from "./globals-BkYSZEKh.js";
import { i as resolveMainSessionKey, t as canonicalizeMainSessionAlias } from "./main-session-ChU7tIhh.js";
import { t as resolveAgentTimeoutMs } from "./timeout-BqDEuVvO.js";
import { i as failTaskRunByRunId, l as startTaskRunByRunId, r as createRunningTaskRun, t as completeTaskRunByRunId } from "./detached-task-runtime-DV0KlEbu.js";
import { a as withAcpRuntimeErrorBoundary, i as toAcpRuntimeError, n as AcpRuntimeError, t as ACP_ERROR_CODES } from "./errors-CKSSmeDI.js";
import { a as identityHasStableSessionId, c as resolveRuntimeHandleIdentifiersFromIdentity, i as identityEquals, l as resolveRuntimeResumeSessionId, n as createIdentityFromHandleEvent, o as isSessionIdentityPending, r as createIdentityFromStatus, s as mergeSessionIdentity, t as createIdentityFromEnsure, u as resolveSessionIdentityFromMeta } from "./session-identity-4aPJNBxa.js";
import { i as requireAcpRuntimeBackend, n as getAcpRuntimeBackend } from "./registry-DipN2vK8.js";
import { i as upsertAcpSessionMeta, n as readAcpSessionEntry, t as listAcpSessionEntries } from "./session-meta-DHsPRv9q.js";
...
const ACP_TURN_STREAM_ABORT_GRACE_MS = 5e3;
const ABORT_SENTINEL = Symbol("acp-turn-stream-abort-fired");
const TIMEOUT_SENTINEL = Symbol("acp-turn-stream-abort-timeout");
async function consumeAcpTurnStream(params) {
	...
	const iterator = params.runtime.runTurn(params.turn)[Symbol.asyncIterator]();
	let abortDeadlineMs;
	...
	if (signal?.aborted === true) {
		...
		abortObserved = true;
	} else if (signal) abortPromise = new Promise((resolve) => {
		abortListener = () => {
			abortDeadlineMs ??= Date.now() + graceMs;
	...
	const requestIteratorReturn = () => {
		if (typeof iterator.return === "function") Promise.resolve().then(() => iterator.return?.(void 0)).catch(() => void 0);
	};
	let pendingNext = null;
	try {
		while (true) {
			if (abortObserved && abortDeadlineMs !== void 0 && Date.now() >= abortDeadlineMs) {
				if (pendingNext) {
					pendingNext.catch(() => void 0);
					pendingNext = null;
				}
			...
			const raceParticipants = [pendingNext];
			if (!abortObserved && abortPromise) raceParticipants.push(abortPromise);
			let timeoutTimer;
			if (abortObserved && abortDeadlineMs !== void 0) {
				const remaining = Math.max(1, abortDeadlineMs - Date.now());
				raceParticipants.push(new Promise((resolve) => {
