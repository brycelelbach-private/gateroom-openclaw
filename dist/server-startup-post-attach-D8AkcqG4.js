...
import { i as getPluginModuleLoaderStats } from "./plugin-module-loader-cache-CRS2pRzy.js";
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-DsqlZqAj.js";
import { t as hasConfiguredInternalHooks } from "./configured-CiZsXTq0.js";
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-DJAm6UXS.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
async function refreshLatestUpdateRestartSentinelIfPresent() {
	...
	return await (await import("./server-restart-sentinel-YAKKaxB2.js")).refreshLatestUpdateRestartSentinel();
}
...
async function hasGatewayStartupInternalHookListeners() {
	const { hasInternalHookListeners } = await import("./internal-hooks-qCbHNo6m.js");
	return hasInternalHookListeners("gateway", "startup");
}
async function waitForAcpRuntimeBackendReady(params) {
	const { getAcpRuntimeBackend } = await import("./registry-CGbEzUpW.js");
...
async function prewarmConfiguredPrimaryModel(params) {
	const { resolveAgentModelPrimaryValue } = await import("./model-input-DIM_MLt8.js");
	const explicitPrimary = resolveAgentModelPrimaryValue(params.cfg.agents?.defaults?.model)?.trim();
	if (!explicitPrimary) return;
	const { normalizeProviderId } = await import("./provider-id-CH0llsyN.js");
	...
	const [{ resolveAgentWorkspaceDir, resolveDefaultAgentDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }, { resolveEmbeddedAgentRuntime }] = await Promise.all([
		import("./agent-scope-CqrunIaJ.js"),
		import("./defaults-C94FBkKL.js"),
		import("./model-selection-CHk7yhsp.js"),
		import("./runtime-JPAULgoV.js")
	]);
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-BMNWWAMu.js");
...
async function startGatewaySidecars(params) {
	await measureStartup(params.startupTrace, "sidecars.gmail-watch", async () => {
		if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-DRO9Dk4_.js");
	...
	await measureStartup(params.startupTrace, "sidecars.gmail-model", async () => {
		if (params.cfg.hooks?.gmail?.model) {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				import("./defaults-C94FBkKL.js"),
				import("./model-catalog-CN1ZGcok.js"),
				import("./model-selection-CHk7yhsp.js")
			]);
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([import("./internal-hooks-qCbHNo6m.js"), import("./loader-Ca5jgTaQ.js")]);
	...
	if (internalHooksConfigured || await hasGatewayStartupInternalHookListeners()) setTimeout(() => {
		import("./internal-hooks-qCbHNo6m.js").then(({ createInternalHookEvent, triggerInternalHook }) => {
	...
	await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-bMfK6BDO.js");
	...
	if (params.cfg.acp?.enabled) (async () => {
		...
		const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] = await Promise.all([import("./manager-Cv1H6QNU.js"), import("./session-identifiers-hE-zRO3S.js")]);
		...
		run: async () => {
			try {
				const [{ resolveStateDir }, { resolveAgentSessionDirs }, { cleanStaleLockFiles }] = await Promise.all([
					import("./paths-B726unMs.js"),
					import("./session-dirs-cF_L043T.js"),
					import("./session-write-lock-CMhULW3N.js")
				]);
				...
				for (const sessionsDir of sessionDirs) {
					...
					if (result.cleaned.length > 0) {
						const { markRestartAbortedMainSessionsFromLocks } = await import("./main-session-restart-recovery-DttCf6BQ.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async () => {
			...
			setTimeout(() => {
				import("./server-restart-sentinel-YAKKaxB2.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-D_DQJQiZ.js");
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleRestartAbortedMainSessionRecovery } = await import("./main-session-restart-recovery-DttCf6BQ.js");
...
const defaultGatewayPostAttachRuntimeDeps = {
	getGlobalHookRunner: async () => (await import("./hook-runner-global-lxRgWryC.js")).getGlobalHookRunner(),
	logGatewayStartup: async (params) => (await import("./server-startup-log-CNd1z-it.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-C-nIMVet.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-CVAU6WLn.js")).startGatewayTailscaleExposure(...args)
};
