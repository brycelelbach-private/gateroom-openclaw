...
import { c as readLocalFileFromRoots, d as basenameFromMediaSource } from "./fs-safe-C91ZNyPb.js";
...
import { r as logVerbose$1 } from "./globals-BkYSZEKh.js";
import { f as resolveOutboundMediaUrls, g as sendMediaWithLeadingCaption, h as resolveTextChunksWithFallback } from "./reply-payload-WdE48c4D.js";
...
import { r as stripMarkdown } from "./text-runtime-BwruZakL.js";
import "./routing-D3J5VUni.js";
import { n as resolveControlCommandGate } from "./command-gating-BM56-zBM.js";
import { d as recordPendingHistoryEntryIfEnabled, l as evictOldHistoryKeys } from "./history-CPJV-NPP.js";
import { t as evaluateSupplementalContextVisibility } from "./context-visibility-COBtwTiW.js";
import "./channel-policy-DRouLMRv.js";
import { n as readStoreAllowFromForDmPolicy, o as resolveDmGroupAccessWithLists, t as DM_GROUP_ACCESS_REASON } from "./dm-policy-shared-B4V1HH_w.js";
...
import { C as formatBlueBubblesChatTarget, L as resolveBlueBubblesEffectiveAllowPrivateNetworkFromConfig, M as resolveBlueBubblesServerAccount, O as normalizeBlueBubblesHandle, P as resolveBlueBubblesAccount, R as resolveBlueBubblesPrivateNetworkConfigValue, S as extractHandleFromChatGuid, T as isAllowedBlueBubblesSender, b as resolveTapbackContext, c as createBlueBubblesClientFromParts, d as buildMessagePlaceholder, f as formatGroupAllowlistEntry, h as normalizeParticipantList, m as formatReplyTag, p as formatGroupMembers, r as isBlueBubblesPrivateApiEnabled, v as parseTapbackText, x as buildBlueBubblesChatContextFromTarget, y as resolveGroupFlagFromChatGuid } from "./probe-DytTINOr.js";
import "./secret-input-Kn4ZxWRH.js";
import { t as getBlueBubblesRuntime } from "./runtime-CAyCqlLo.js";
import { i as resolveBlueBubblesInboundConversationId } from "./conversation-id-Bx-OTS_O.js";
import { _ as fetchBlueBubblesMessageAttachments, a as resolveBlueBubblesMessageId, b as sendMessageBlueBubbles, g as downloadBlueBubblesAttachment, i as rememberBlueBubblesReplyCache, n as sendBlueBubblesReaction, o as resolveReplyContextFromCache, p as sendBlueBubblesTyping, r as getShortIdForUuid, t as normalizeBlueBubblesReactionInputStrict, u as markBlueBubblesChatRead, v as sendBlueBubblesAttachment, y as resolveChatGuidForTarget } from "./reactions-C-9vZCVo.js";
...
//#endregion
//#region extensions/bluebubbles/src/monitor-reply-fetch.ts
const DEFAULT_REPLY_FETCH_TIMEOUT_MS = 5e3;
...
const PART_INDEX_REPLY_TO_ID_MAX_LENGTH = REPLY_TO_ID_MAX_LENGTH + 2 + 10 + 1;
/**
...
*/
function fetchBlueBubblesReplyContext(params) {
	const replyToId = sanitizeReplyToId(params.replyToId);
	if (!replyToId || !params.baseUrl || !params.password) return Promise.resolve(null);
...
*/
function sanitizeReplyToId(raw) {
	const trimmed = raw.trim();
	if (!trimmed) return null;
	const bare = trimmed.includes("/") ? trimmed.split("/").pop() ?? "" : trimmed;
	if (!bare || bare.length > REPLY_TO_ID_MAX_LENGTH || !REPLY_TO_ID_PATTERN.test(bare)) return null;
	return bare;
}
function normalizePartIndexReplyToIdAlias(raw, bareReplyToId) {
	const trimmed = raw.trim();
	if (trimmed.length > PART_INDEX_REPLY_TO_ID_MAX_LENGTH) return null;
	const match = PART_INDEX_REPLY_TO_ID_PATTERN.exec(trimmed);
	if (!match || match[1] !== bareReplyToId) return null;
	return trimmed;
}
async function runFetch(params, replyToId) {
	const client = (params.clientFactory ?? createBlueBubblesClientFromParts)({
		accountId: params.accountId,
	...
	try {
		...
		if (partIndexReplyToId) rememberBlueBubblesReplyCache({
			...cacheEntry,
...
function extractBody(data) {
	return normalizeOptionalString(data.text) ?? normalizeOptionalString(data.body) ?? normalizeOptionalString(data.subject);
}
...
function extractSender(data) {
	...
	const raw = normalizeOptionalString(handle?.address) ?? normalizeOptionalString(handle?.id) ?? normalizeOptionalString(data.senderId) ?? normalizeOptionalString(data.sender);
	if (!raw) return;
...
async function processMessageAfterDedupe(message, target, dedupeSignal) {
	...
	if (attachments.length > 0) if (!baseUrl || !password) logVerbose(core, runtime, "attachment download skipped (missing serverUrl/password)");
		...
		try {
			...
			runtime.error?.(`[bluebubbles] attachment download failed guid=${safeGuid} err=${safeErr}`);
	...
	if (replyToId && (!replyToBody || !replyToSender) && baseUrl && password && account.config.replyContextApiFallback === true) {
		...
		if (fetched) {
			if (!replyToBody && fetched.body) replyToBody = fetched.body;
			if (!replyToSender && fetched.sender) replyToSender = fetched.sender;
			if (core.logging.shouldLogVerbose()) {
				const preview = sanitizeForLog((fetched.body ?? "").replace(/\s+/g, " "), 120);
				logVerbose(core, runtime, `reply-context API fallback replyToId=${sanitizeForLog(replyToId)} sender=${sanitizeForLog(fetched.sender ?? "")} body="${preview}"`);
			}
		}
	}
	...
	const prepareBlueBubblesReplyPayload = (payload) => {
	...
	const canUseDurableBlueBubblesFinalDelivery = (payload) => {
				...
				resolveTurn: () => ({
					...
					delivery: {
						...
						durable: (payload, info) => {
							if (info.kind !== "final" || !canUseDurableBlueBubblesFinalDelivery(payload)) return false;
							const replyToMessageGuid = resolveReplyToMessageGuidForPayload(payload);
							return {
								to: outboundTarget,
								replyToId: typeof payload.replyToId === "string" ? payload.replyToId.trim() || null : null,
								deps: { bluebubblesMessageLifecycle: {
									beforeSendAttempt: (ctx) => {
										const snippet = ctx.kind === "media" ? (ctx.text ?? "").trim() || "<media:attachment>" : (ctx.text ?? "").trim();
									...
									afterSendSuccess: (ctx) => {
										const snippet = ctx.kind === "media" ? (ctx.text ?? "").trim() || "<media:attachment>" : (ctx.text ?? "").trim();
										if (maybeEnqueueOutboundMessageId(ctx.result?.messageId, snippet) && typeof ctx.attemptToken === "number") forgetPendingOutboundMessageId(ctx.attemptToken);
									},
									afterSendFailure: (ctx) => {
										if (typeof ctx.attemptToken === "number") forgetPendingOutboundMessageId(ctx.attemptToken);
									}
								} },
						...
						onDelivered: (_payload, info, result) => {
							if (!result?.deliveryIntent) return;
					...
					replyPipeline: { typingCallbacks },
					dispatcherOptions: {
						onReplyStart: typingCallbacks.onReplyStart,
						onIdle: typingCallbacks.onIdle
					},
					replyOptions: { disableBlockStreaming: typeof account.config.blockStreaming === "boolean" ? !account.config.blockStreaming : void 0 },
