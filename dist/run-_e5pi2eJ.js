...
import { t as CONFIG_PATH, u as resolveGatewayPort, v as resolveStateDir } from "./paths-BplLTi2s.js";
...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-DlmlcGrT.js";
...
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-D8Y23pOF.js";
import { t as parsePort } from "./parse-port-BXym6Fwj.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-D0ykAgvP.js";
import { r as withProgress } from "./progress-DvjmBQTm.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-2qjvNtak.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-DLiUEXqB.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-BZqF1fzG.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-DoA2fCY5.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-CI9tLRel.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BGPJi1Db.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-Ck45CVhy.js");
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-Dj9mijg5.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-DG97x1w1.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-BJeSYBcu.js");
	...
	const { detectRespawnSupervisor } = await import("./supervisor-markers-pSoP72Cf.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-CMaDMwQP.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-BRMpYfy4.js");
